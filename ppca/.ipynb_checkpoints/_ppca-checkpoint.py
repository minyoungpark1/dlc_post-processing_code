...
class PPCA():
    ...
    def inverse_transform(self):
        ...
        Sx = np.linalg.inv(np.eye(d) + CC/self.ss)
        X = np.dot(np.dot(self.data, self.C), Sx) / self.ss
        XX = np.dot(X.T, X)
        C = np.dot(np.dot(self.data.T, X), np.linalg.pinv(XX + N*Sx))
        CC = np.dot(C.T, C)
        recover_ppca = np.dot(X, C.T)
        # recover_ppca = recover_ppca*self.stds + self.means
        
        # recover_ppca = np.dot(self.data, self.C)
        # recover_ppca = recover_ppca*self.stds + self.means
        return recover_ppca
