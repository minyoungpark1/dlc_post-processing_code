...
def extract_frames(vidpath, times, every_n_frames, path_to_save, img_format):
    ...
    with tqdm(total=count) as pbar:
        for start_time, end_time in times:
            ...
            for frame_count in frame_counts:
                ...
                ret, frame = cap.read()
                cv2.imwrite(os.path.join(path_to_save, 'img' +
                                         str(frame_count).zfill(6) +
                                         '.' + img_format), frame)
                pbar.update(1)
...
def extract_specific_frames(vidpath, frame_counts, path_to_save, img_format):
    if not os.path.exists(vidpath):
        print('Video does not exist.')
        return
    ...
    for frame_count in tqdm(frame_counts):
        cap.set(cv2.CAP_PROP_POS_FRAMES, frame_count)
        ret, frame = cap.read()
        cv2.imwrite(os.path.join(path_to_save, 'img' +
        str(frame_count).zfill(5) + '.' + img_format), frame)
...
def generate_three_dim_video(config, **kwargs):
    ...
    three_dim_video_filename = '3d-video.mp4'
