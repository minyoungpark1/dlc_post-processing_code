...
def load_calib_new(config):
    ...
    calib = toml.load(os.path.join(config['calibration']['calib_video_path'], 'calibration.toml'))
    keys = list(calib.keys())[:-1]
    intrinsics = {}
    extrinsics = {}
    for idx, key in zip(vid_indices, keys):
        intrinsics[idx] = {'camera_mat': calib[key]['matrix'],
                           'dist_coeff': calib[key]['distortions']}
        
        extrinsics[idx] = make_M(arr(calib[key]['rotation']), 
                                 arr(calib[key]['translation']))
